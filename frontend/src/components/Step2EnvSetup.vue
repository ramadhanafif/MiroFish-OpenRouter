    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Pulls entities and relationships from the knowledge graph to create the simulated individuals, giving each one unique behaviors and memories grounded in the reality seed
          </p>
          ...
          <div v-if="profiles.length > 0" class="stats-grid">
            ...
            <div class="stat-card">
              ...
              <span class="stat-label">Related Topics in Reality Seed</span>
            </div>
          </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            The LLM uses the simulation requirements and reality seed to configure the world: time flow rate, recommendation algorithm, each individual's active hours and posting frequency, event triggers, and more
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            Generates the initial activation events and trending topics from the narrative direction, seeding the simulated world's starting state
          </p>
          ...
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                ...
                <span class="section-desc">MiroFish plans to simulate <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> hours of real time; each round advances the clock by <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> minutes</span>
              </div>
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                <div class="slider-display">
                  ...
                  <div class="slider-meta-info">
                    <span>With 100 agents: estimated runtime ~{{ Math.round(customMaxRounds * 0.6) }} minutes</span>
                  </div>
                </div>
                  ...
                  <div class="auto-content">
                    <div class="auto-meta-row">
                      <span class="duration-badge">
                        ...
                        With 100 agents: estimated runtime ~{{ Math.round(autoGeneratedRounds * 0.6) }} minutes
                      </span>
                    </div>
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">First run? Switch to Custom and lower the rounds for a quick preview with less risk of errors ➝</p>
                    </div>
                  </div>
                </div>
              </div>
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.already_prepared) {
        addLog('Found existing completed preparation, reusing it')
      ...
      if (res.data.expected_entities_count) {
        ...
        addLog(`Read ${res.data.expected_entities_count} entities from the Neo4j graph`)
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (currentCount > 0 && currentCount !== lastLoggedProfileCount) {
        ...
        addLog(`→ Persona ${currentCount}/${total}: ${profileName} (${latestProfile?.profession || 'unknown profession'})`)
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        addLog('Configuration is still generating, polling until it completes...')
        startConfigPolling()
      }
    }
  } catch (err) {
